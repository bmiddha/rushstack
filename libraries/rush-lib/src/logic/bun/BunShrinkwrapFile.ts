...
import { IParsedPackageNameOrError, InternalError } from '@rushstack/node-core-library';
...
/**
 * Used with BunShrinkwrapFile._encodePackageNameAndSemVer() and _decodePackageNameAndSemVer().
 */
...
/**
 * Part of IBunShrinkwrapJson
 */
interface IBunShrinkwrapEntry {
  /**
   ...
   *
   * Examples:
   * https://registry.bunpkg.com/@pnpm/types/-/types-1.7.0.tgz#9d66a8bed3fabcd80f288b3e7884b7418b05b5a9
...
/**
 * Used by BunShrinkwrapFile to interpret the `@bunpkg/lockfile` data structure.
 */
interface IBunShrinkwrapJson {
  /**
   * Example keys:
   ...
   */
  [packageNameAndSemVer: string]: IBunShrinkwrapEntry;
}

/**
 * Support for consuming the "bun.lock" file.
 *
 * Bun refers to its shrinkwrap file as a "lock file", even though it has nothing to do
 ...
 * generically to bun.lock as a "shrinkwrap file".
 *
 * If Rush's Bun support gains popularity, we will try to improve the wording of
 * logging messages to use terminology more consistent with Bun's own documentation.
 */
export class BunShrinkwrapFile extends BaseShrinkwrapFile {
  ...
  private _shrinkwrapJson: IBunShrinkwrapJson;
  private _tempProjectNames: string[];

  private constructor(shrinkwrapJson: IBunShrinkwrapJson) {
    super();
    ...
    for (const key of Object.keys(this._shrinkwrapJson)) {
      // Example key:
      const packageNameAndSemVer: IPackageNameAndSemVer = BunShrinkwrapFile._decodePackageNameAndSemVer(key);
      ...
      if (
        ...
        if (!/^file:/i.test(packageNameAndSemVer.semVerRange)) {
          ...
          throw new Error(
            'Unexpected package/semver expression found in the Bun shrinkwrap file (bun.lockb): ' +
              JSON.stringify(key)
          );
        }
        ...
        if (!seenEntries.add(packageNameAndSemVer.packageName)) {
          ...
          throw new Error(
            'Duplicate @rush-temp package found in the Bun shrinkwrap file (bun.lockb): ' +
              JSON.stringify(key)
          );
        }
        ...
        const entry: IBunShrinkwrapEntry = this._shrinkwrapJson[key];

        // Bun fails installation if the integrity hash does not match a "file://" reference to a tarball.
  ...
  /**
   * The `@bunpkg/lockfile` API only partially deserializes its data, and expects the caller
   * to parse the bun.lockb lookup keys (sometimes called a "pattern").
   *
  ...
  private static _decodePackageNameAndSemVer(packageNameAndSemVer: string): IPackageNameAndSemVer {
    const result: RegExpExecArray | null =
      BunShrinkwrapFile._packageNameAndSemVerRegExp.exec(packageNameAndSemVer);
    if (!result) {
      // Sanity check -- this should never happen
      throw new Error(
        'Unable to parse package/semver expression in the Bun shrinkwrap file (bun.lockb): ' +
    ...
    if (parsedPackageName.error) {
      // Sanity check -- this should never happen
      throw new Error(
        'Invalid package name the Bun shrinkwrap file (bun.lockb): ' +
          JSON.stringify(packageNameAndSemVer) +
          '\n' +
   ...
   * Given an IPackageNameAndSemVer object, recreate the bun.lockb lookup key
  ...
  public hasCompatibleTopLevelDependency(dependencySpecifier: DependencySpecifier): boolean {
    // It seems like we should normalize the key somehow, but Bun apparently does not
    // do any normalization.
    const key: string = BunShrinkwrapFile._encodePackageNameAndSemVer({
  ...
  ): BaseProjectShrinkwrapFile<BunShrinkwrapFile> | undefined {
    return undefined;
  }
